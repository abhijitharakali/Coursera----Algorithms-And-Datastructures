...
public class Debug_DistWithCoords1{   
    ...
    public double pot(int u, int v, int node){
...
//        int m_max = n*(n-1)/2;
        ...
        while(eflag){
            ...
            for(int i = 0; i < m; i++){
                ...
                double tempdist = Math.sqrt((x[s] - x[t])*(x[s] - x[t]) + (y[s] - y[t])*(y[s] - y[t]));
                int dtemp = (int) Math.floor(tempdist+1);
                if((int)(0.3*dtemp) > 0){
            ...
            for(int u = 0; u < n; u++){
                for(int v = 0; v < n; v++){
                    if((u!=v)){
                        long bq = bug.query(u, v);
                        long rq = ref.query(u, v);
                            if(bq != rq){
                                System.out.println(bq+" "+rq);
                                System.out.println();
                                eflag = false;
