...
/*
This version barely passed. The last passed version (Ver3) was all flat.
I like the modular code so I tried to keep it that way. The other improvements
present in Ver3 are also present in this code.
*/

public class FriendSuggestion_Efficient_Passed_Ver4{
    ...
    public static void Process(int node, ArrayList<Integer>[] adj, ArrayList<Integer>[] cost,PriorityQueue<Entry> q, HashMap<Integer,Long> distq, HashSet<Integer> workset){
    ...
    public static long query(ArrayList<Integer>[] adjf, ArrayList<Integer>[] adjr, ArrayList<Integer>[] costf, ArrayList<Integer>[] costr,int u,int v){
        ...
        while(!qf.isEmpty() && !qr.isEmpty()){
            ...
            if(wf.contains(uf) == false){
                wf.add(uf);
                Process(uf, adjf, costf, qf, distf, wf);
            ...
            if(wr.contains(vr) == false){
                wr.add(vr);
                Process(vr, adjr, costr, qr, distr, wr);
